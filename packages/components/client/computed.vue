  ...
  <div class="k-schema-group" v-if="isSwitch">
    ...
    >
      <template #menu>
        ...
        <el-dropdown-item :disabled="index === modelValue.$switch.branches.length - 1" @click="actions.down(index)">下移</el-dropdown-item>
...
const actions = {
  ...
  add() {
    if (props.modelValue.value?.$switch) {
      const branches = props.modelValue.$switch.branches.slice()
      ...
      emit('update:modelValue', {
        $switch: {
          ...
          default: clone(modelValue.value),
        },
      })
    }
  },
